...
* 모든 옵션의 앞에는 하이픈(-)이 있어야한다. 이중 하이픈도 사용(--opt)

* 인자가 없는 옵션은 하나의 - 다음에 묶여서 올 수 있다.(예: -abc)

* 옵션의 첫 번째 인자는 공백이나 탭으로 띄고 입력해야 한다.(예: -l /usr/bin)

* 인자가 있어야 하는 옵션에서 인자를 생략할 수 없다.

* 모든 옵션은 명령의 인자보다 앞에 와야한다.(예: ls -a /usr/bin)

* 옵션의 끝을 나타내기 위해 --을 사용할 수 있다
